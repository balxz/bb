...
const makeMessagesSocket = (config) => {  
    ...
    const bail = {
        generateWAMessageContent: Utils_1.generateWAMessageContent,
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, statusJidList, AI = true }) => {  
        ...
        await authState.keys.transaction(async () => {  
            ...
            if (AI && isPrivate) {  
                ...
                const filteredBizBot = false; // WABinary_1.getBinaryFilteredBizBot(additionalNodes ? additionalNodes : [])
    ...
    const handleGroupStory = async (content, jid, quoted) => {
        ...
        try {
            ...
            const messageId = bail.generateMessageID();
            return await relayMessage(jid, msg.message, {
                messageId: messageId
            });
        } catch (error) {
            logger.error({ error }, 'Error handling group story');
            throw error;
        }
    };
    ...
    const handleAlbumMessage = async (content, jid, quoted) => {  
        ...
        const album = await Utils_1.generateWAMessageFromContent(jid, {  
            ...
            userJid: (0, Utils_1.generateMessageID)().split('@')[0] + '@s.whatsapp.net',  
            quoted,  
            upload: waUploadToServer  
        });  
            ...
            await relayMessage(jid, img.message, {  
                ...
                quoted: {  
                    key: {  
                        ...
                        id: album.key.id,  
                        fromMe: true,  
                        participant: (0, Utils_1.generateMessageID)().split('@')[0] + '@s.whatsapp.net',  
                    },  
        ...
        sendMessage: async (jid, content, options = {}) => {  
            ...
            if (messageType) {  
                switch(messageType) {  
                    ...
                    case 'GROUP_STORY':
                        return await handleGroupStory(content, jid, quoted);
                }  
            }  
